...
export function useAccountWolfis(forceRefresh?: number) {
  ...
  const getData = useCallback(async () => {
    if (account) {
      setLoading(true)
      try {
        const mintedWolfies = (await mintingContract?.walletOfOwner(account)).map((wolfi: any) => ({
          wolfi,
        }))
        ...
        const metadatas = await Promise.all(ipfs.map((ipfsuri: string) => fetch(ipfsuri)))

        const jsonmetadatas = await Promise.all(metadatas.map((metadata: any) => metadata.json()))

        const images = jsonmetadatas.map((jsonmeta: any) => fromIpfsToUrl(jsonmeta.image))

        const imagesWithIpfsUri = ipfs.map((ipfsuri: string, index: number) => ({
          image: images[index],
          ipfsuri,
